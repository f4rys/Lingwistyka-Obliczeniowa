...
def measure_generation_latency(
    ...
    *,
    ...
    """Return avg milliseconds per generated token for greedy/top-k sampling.

    Uses random integer prompts to avoid tokenizer dependency.
    """
    device = next(model.parameters()).device
    prompt = torch.randint(0, vocab_size, (1, prompt_len), device=device, dtype=torch.long)
    # warmup
...
def measure_training_step_time(
    ...
    *,
    ...
    """Return average milliseconds per train step (forward+backward+step) on synthetic data."""
    device = next(model.parameters()).device
    model.train()
